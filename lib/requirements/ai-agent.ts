...
function buildStructuredPrompt(request: RequirementAgentRequest, agentToolContext?: string) {
  return [
    ...
    "- If the message or developerIntent says this is a final SRS generation request, you MUST return exactly zero clarification questions and you MUST make developer_srs the primary artifact with non-empty requirements. Do not block the request for any reason. The developer_srs artifact content must be clean markdown prose in plain English sections, not JSON, not escaped JSON, not Mermaid, and not random placeholder text.",
